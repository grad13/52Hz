...
### Added
- 多重起動防止 (`tauri-plugin-single-instance`)。`com.hz52.app` の 2 回目の起動を抑制する。コールバックは debug ビルドで `single-instance: suppressed a duplicate launch attempt` を stderr に記録するのみで、main window は表示しない (52Hz は tray 駆動でウィンドウは tray クリック時にのみ位置調整付きで開く)。`FIFTYTWOHZ_HEADLESS` 時は登録をスキップし、統合テストの連続 spawn を妨げないようにしている
- `media.rs` に MediaRemote 単体テスト 4 件 (`MR_TOGGLE_PLAY_PAUSE` 値、`MEDIA_REMOTE_PATH`、dlopen+dlsym 解決、`last_dl_error` の NULL 安全性)

### Fixed
- CI 統合テストの失敗を修正。`event_handlers.rs` の break-start / break-end ログを `log::info!` から `eprintln!` に戻した。テストは child 子プロセスの stderr を走査するが、`tauri-plugin-log` のデフォルト target は Stderr を含まないため、`log::info!` のままだと `"opening overlay"` などの assertion が通らなかった

### Changed
- `documents/spec/tauri/media.md` を MediaRemote 実装に合わせて全面書き換え
- `documents/spec/tauri/lib.md` の §3.5 / §3.6 を `media::post_play_pause_key` 単一呼び出しに更新、§4.5 / §4.6 に `tauri-plugin-single-instance` の説明と `FIFTYTWOHZ_HEADLESS` での skip を追記

## [0.8.0] - 2026-05-08

### Changed
- 休憩中のメディア一時停止を MediaRemote.framework に刷新。Firefox を含むあらゆるブラウザの動画/音声と Spotify / Music / Podcasts が対象に
- 旧実装（Spotify/Music の AppleScript と Chrome 系での `'k'` キー送信）を全削除し、フォーカスを奪わない単一経路に集約
- Accessibility 権限は不要になった
- リリースビルドでも `tauri-plugin-log` を有効化し、`~/Library/Logs/com.hz52.app/52Hz.log` にメディア制御の試行/結果を記録

## [0.7.3] - 2026-03-17

### Changed
- focus 終了時の休憩/skip パネルを message OFF でも常に表示するように変更
- 休憩/skip パネルから時間延長ボタン（+1m, +3m, +5m）を削除
