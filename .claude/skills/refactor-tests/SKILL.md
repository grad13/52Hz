...
2. **対象選定**: `tests/unit/` の `*.test.{ts,js}` を収集
...
- **vitest統一**: 非標準ランナーは移行対象
- **@codeエイリアス統一**: import先はプロジェクト規約に従う
- **tsc ゲート**: 分割後に `npx tsc --noEmit` で新規エラーがないことを確認
- **mock<T>() 統一**: interface モックは `vitest-mock-extended` の `mock<T>()` を使用。手書き部分モック禁止
...
| **should** | S2 | 非標準ランナー | vitest以外のランナー（node:test等）を使用 |
| **should** | S3 | import先不正 | `@code` エイリアス未使用 |
| **should** | S6 | 責務混在 | 複数モジュールを1テストファイルでテスト |
| **should** | S7 | 手書き部分モック | `mock<T>()` ではなく手書きの部分オブジェクトで interface をモック |
| **should** | S8 | tsc エラー | `npx tsc --noEmit` でエラーが出る |
...
`tests/unit/` 配下の `*.test.{ts,js}` を Glob で収集する。
...
├── must/                         # must判定（必須対象）
│   └── unit/path/to/file.test.ts.md   # 元のパス構造を保持
├── should/                       # should判定（推奨対象）
│   └── unit/path/to/file.test.ts.md
└── summary.md
```

**重要: 出力パスは元のtests/以下の構造を保持する。**
- 例: `tests/unit/api/helpers.test.ts` → `tests/.refactor-tests/must/unit/api/helpers.test.ts.md`
...
2. 基本情報を確認:
   ...
   - テストケース数: describe/it/test を数える
   - import文の確認（@code エイリアス使用有無、import先モジュール数）
3. 問題点チェック:
   - M2: 500行超？
   - S2: vitest以外のランナーを使用しているか？
   - S3: @code エイリアスを使用しているか？
   - S6: 複数モジュールを1ファイルでテストしていないか？（import文から判定）
   - S7: mock<T>() ではなく手書き部分モックを使っているか？
   - S8: tsc エラーがあるか？（ファイル内の型記述から推定）
4. 問題があれば:
   - 問題点を詳細分析
...
```
must:unit/api/helpers.test.ts
should:unit/utils/format.test.ts
clean:unit/core/parser.test.ts
```

**パス構造保持**: 出力ファイルは元のtests/以下の構造を維持する。
- 入力: `tests/unit/api/helpers.test.ts`
- 出力: `tests/.refactor-tests/must/unit/api/helpers.test.ts.md`
