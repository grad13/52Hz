...
| 出力 | `_documents/.spec-from-code/` → 確認後 `_documents/spec/` |
...
```
code-to-spec code/tauri/src
code-to-spec code/frontend/components
```
...
`code/.refactor/summary.md` からファイルリストを取得。
...
**--from-refactorの場合:**
```bash
# code/.refactor/summary.md を読み込み
...
1. 形式を決定（判定基準参照）
2. 完全なspecを生成
3. `_documents/.spec-from-code/generated/{カテゴリ}/{ファイル名}.md` に出力

#### partialの場合（追記用spec）

1. 既存specの形式を確認
2. 不足している仕様のみを抽出・生成
3. `_documents/.spec-from-code/generated/{カテゴリ}/{ファイル名}-supplement.md` に出力
4. フロントマターに `Type: Supplement` と `Target: {既存specパス}` を記載

### Step 4: サマリー生成

`_documents/.spec-from-code/summary.md` を生成:
...
2. **新規specの説明**（各ファイルについて）
   - 対象コード
   - 形式: spec-v2.1（Contract + State + Logic + Side Effects 同梱）
...
1. **新規spec**: `.spec-from-code/generated/` から `_documents/spec/` にコピー
...
│       │   └── overlay.md            # 新規spec
│       ├── presence/
│       │   └── presence-supplement.md  # 追記用spec
│       └── tray/
│           └── tray.md               # 新規spec
...
| Format | spec-v2.1（Contract + State + Logic + Side Effects 同梱） |
| Source | code/ からの相対パス |
...
## 入力
- コードファイル: {絶対パス}
- specディレクトリ: {_documents/spec/の絶対パス}

## 手順
1. コードをReadで読み込む
2. 公開インターフェースを特定:
   - 関数名、構造体名、トレイト名（Rust）
   - export された関数/クラス/定数（TypeScript/Svelte）
3. _documents/spec/ 内で関連specを検索:
   - Grepでファイル名・関数名を検索
   - 見つかったファイルを読んで確認
4. カバレッジを判定:
