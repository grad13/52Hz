...
5. **サマリー**: 統計・一覧を `_documents/.spec-from-tests/summary.md` に出力
6. **ユーザー確認**: 概要提示 → 許可取得
7. **適用**: 許可後に `_documents/spec/` へ配置 + テストの `Spec:` ヘッダー追加
...
| 出力 | `_documents/.spec-from-tests/` → 確認後 `_documents/spec/` |
...
3. **対応 spec を検索**
   - `_documents/spec/` 内で Grep（ソースファイル名、モジュール名）
   - 見つかった場合は読み込み
...
**結果出力:** `_documents/.spec-from-tests/analysis/` に書き出す。
...
3. `_documents/.spec-from-tests/generated/{カテゴリ}/{ファイル名}.md` に出力
...
2. `_documents/.spec-from-tests/generated/{カテゴリ}/{ファイル名}-supplement.md` に出力
...
2. `_documents/.spec-from-tests/generated/{カテゴリ}/{ファイル名}.md` に出力

### Step 4: サマリー生成

`_documents/.spec-from-tests/summary.md` を生成:
...
1. **Case A**: `.spec-from-tests/generated/` から `_documents/spec/` にコピー
...
1. `_documents/.spec-from-tests/` ディレクトリを削除
...
3. _documents/spec/ 内で関連specをGrepで検索、見つかればReadで読む
...
- **Spec ヘッダー追加**: 適用後、対応するテストファイルに `// Spec: _documents/spec/{path}` ヘッダーを追加
