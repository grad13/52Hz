...
```
tests-to-spec tests/unit/utils
tests-to-spec tests/unit/journal
```

指定ディレクトリ内のテストファイル（`*.test.{ts,js}`）を対象とする。
...
**ディレクトリ指定の場合:**
- 指定ディレクトリ内の `*.test.{ts,js}` を収集
- 除外: `_archive/`, `node_modules/`, `.refactor-tests/`, `.tests-from-spec/`
...
1. **テストファイルを読み込み**
   - describe/it 構造の把握
   - import 文からテスト対象ソースを特定
   - `Spec:` ヘッダーの有無
   - テストケース数

2. **ソースコードを読み込み**（補助情報）
   - 公開 IF の型情報
   - Runtime 判定（JS-ESM / JS-CJS / JS-IIFE / PHP）
...
## 手順
1. テストファイルをReadで読み込む
2. import文からテスト対象のソースファイルを特定し、Readで読み込む
...
4. spec-v2.1 形式で spec を生成:
   ...
   ### 0. Meta
   - Source: import パスから特定
   - Runtime: ソースコードの形式から判定
   - Test Type: Unit

   ### 1. Contract
   - テストの import 型、mock<T>() の型引数 → interface 定義
   - ソースコードの export 型で補完
   - テストが検証している公開 IF のみ記載

   ### 2. State
   - describe のネスト構造から状態遷移を推定
   - 状態遷移がないモジュール（純粋関数等）は省略可

   ### 3. Logic
   - 各 it() を Decision Table の1行に変換
   - テスト名 → Case ID + 説明
   - テストの入力値 → Input
   - expect の期待値 → Expected

   ### 4. Side Effects
   - vi.fn()、mock<T>() の対象 → 副作用一覧
