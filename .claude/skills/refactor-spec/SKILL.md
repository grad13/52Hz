...
結果は `_documents/.refactor-spec/summary.md` を参照。
...
`_documents/spec/` 配下の `*.md` を Glob で収集する。
...
全サブエージェント完了後、`_documents/.refactor-spec/summary.md` を生成。
...
**重要: 出力パスは元の_documents/以下の構造を保持する。**
- 例: `_documents/spec/api/auth.md` → `_documents/.refactor-spec/must/spec/api/auth.md.md`
...
各サブエージェントは分析結果を `_documents/.refactor-spec/` に書き出し、**最小限の戻り値**を返す:
...
**パス構造保持**: 出力ファイルは元の_documents/以下の構造を維持する。
- 入力: `_documents/spec/api/auth.md`
- 出力: `_documents/.refactor-spec/must/spec/api/auth.md.md`
...
- **ファイル出力必須**: must/shouldの場合、必ず `_documents/.refactor-spec/` にWriteツールでファイルを書き出す
- **パス構造保持**: 出力ファイルは元の_documents/以下のディレクトリ構造を維持する
